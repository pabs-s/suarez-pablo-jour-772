...
**Answer**
2016 and 2018 have the same column structure and order. 2020 and 2022 have the same column headings as the previous two datasets, but the columns are listed in a different order. For 2022 specifically, it's "total" column is undercase while all other files have that title in uppercase.
...
**Answer** 
All of the files have been merged into one file that shares the column structure and order as the 2016 and 2018 files. It is organized in descending order based on year and there are two columns for TOTAL and total. Only the 2022 figures fall under the lowercase version because that is how it was listed in only that file.
...
**Answer**
I'm most interested in seeing the significant increase in total voters as a whole. This increase seems to benefit the Democratic Party the most as it gained almost 18,000 voters. The next highest were unaffiliated voters, followed by a much smaller increase with the Republican Party. The rest, unsurprisingly to me, seem to be pretty stable.
```{r}
...
**Answer**
The data from the previous task suggests there should be 24 rows. After running this code, there are 24 rows.
...
**Answer**
This code created a new dataframe merging the population data with voter registration data, with each row representing a different county.
...
**Answer**
This file contains population demographic information by county. It shows the geocode for each county, the number of people in each county broken down by race, as well as the number of people above the age of 18.
...
**Answer**
Prince George's	County had the highest 18+ Black population with 60.9035922%, and had the highest percentage of Democrats with 77.09508%.	
...
**Answer**
Howard County is the county with the highest 18+ Asian population as a percentage of all population at 19.367% and its percentage of Democrats is 52.57%. The percentage of Asian residents is significantly lower than the percentage of Black residents. It's also harder to get a sense of how Asian residents contribute to the percentage of Democratic voters.
...
**Answer**
The ZCTA5N column is a dbl datatype, suggesting that it's a numeric column. However, zip codes and ZCTAs are better suited as characters datatypes.
...
**Answer**
After running the code, the ZCTA5N column's datatype is now a character-type.
