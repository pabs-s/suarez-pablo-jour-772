...
### Task 5: Use refinr to clean the grantee_clean field.
	...
	**Task** Run the following code to make a copy of the Grantee field called grantee_clean, then use the `key_collision_merge` function from refinr to attempt to standardize similar values. Store it as a new object called cleaned_md_grant_loans Click the black arrow on the table to examine the Grantee and grantee_clean columns. Looking at the results on the first 3 pages only (first 30 rows), name two changes the refinr package made, by comparing Grantee to grantee_clean. **Answer: Standardized formatting for names like "WASHINGTON COUNTY MUSEUM OF FINE ARTS, THE TREE HOUSE CHILD ADVOCACY CENTER OF MONTGOMERY COUNTY MD INC." It also matched punctuation between the original and clean names.**
...
**Task** Run the following code to use the n_gram_merge() function to clean the data, then group by grantee_clean and Grantee to examine recipients that start with W. Look at the differences. Does the changes it made vary substantially from the previous use of key_collision_merge? **Answer: I don't see a substantial change between the two datasets.**
