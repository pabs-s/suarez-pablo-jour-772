...
## The summary function gives me information about each column, depending on the data type (characters/strings vs integers/floats). The colnames function provides the names of each column in the data sheet.
...
**Answer: It's important because it simplifies the dataset by not having to run that code every single time. The user can now reference that specific data using the new variable.**
...
**Answer: The data type changed from a character column to a date column. It also changed the formatting to year-month-day.**
...
**Answer: The values in the month column are arranged by total_amount in descending order. It seems like it uses the first day of the month for each month when it is looking for sum amounts.**
...
**Answer: The code is calculating the total number votes between the candidates and then dividing that total from the total of Wes Moore's votes to find his percentage of votes won. It then mutates the table to show this percentage in a new column.**
...
**Answer: The answer changed because the values in the pct_moore column have been turned into percentages by moving the decimal point two places to the right.**
...
**Answer: This is an interesting answer because it helps us identify state counties where Moore was more highly contested.**
...
**Answer: str_to_title() , which converts strings to title case, where only the first letter of each word is capitalized. str_to_sentence() which changes strings to where only the first letter of sentence is capitalized.**
...
**Answer: The code is searching the address strings for the specific state names listed below. If it finds those states in the address, then it's using the corresponding string state abbreviations in a new column titled 'state'.**
...
**Answer: This answer makes sense in that we are seeing the total amount of payments coming from different states. Those that fall under NA are payments that came from states not included in the previous case_when function.**
...
1.  How could we arrange the table to see total amount by expense method and state?

2.  For the UMD course data, how can we mutate the courses to include honors classes in the same categories as their regular variants?

3.  How might I count the number of terms a course has been offered in at UMD and also see during which terms were they were offered?
