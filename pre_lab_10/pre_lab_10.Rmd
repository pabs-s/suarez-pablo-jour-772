...
**Answer**
I see columns for objectid, two different zip code columns, shape length and area columns, and geometry.
...
**Answer**
It's generally the same as the example above. Zip codes, shape length, shape area, geometry, etc.
...
**Answer**
I see a map of Maryland, with presumably the outlines of each county.
...
**Task** Run the following code. Describe the output in the space below. 
**Answer**
I didn't see anything notable other than the fact that tigris was installed.
...
**Task** Run the following code. Describe the output in the space below. 
**Answer**
I see a lot of the same columns that have appeared in the codeblocks above. There are several new columns with names or acronyms that are unfamiliar to me. Some of those include CLASSFP, MTFCC, CSAFP, CBSAFP, METDIVFP, etc. There's also land and water data, as well as a geometry column.
...
**Task** Run the following code. Describe the output in the space below. 
**Answer**
Most of the columns we just saw in the codeblock above are in this dataframe. To include, CLASSFP, MTFCC, CSAFP, CBSAFP, METDIVFP, and land, water and geometry.
...
**Task** Run the following code. Describe the output in the space below. 
**Answer**
A map of Maryland with counties and latitude and longitude.
...
**Task** Run the following code. Describe the output in the space below. 
**Answer**
It's the same map as before, but now we've added the zip code boundaries back in (and maybe water as well?).
...
**Task** Run the following code. Describe the output in the space below. 
**Answer**
The output of this codeblock is a two-column dataframe stating the number of foreclosures within each zip code during March 2023.
...
**Task** Run the following code. Describe the output in the space below. 
**Answer**
We've added the foreclosure data to a dataframe with our shapes/geometric map data. 
...
**Task** Run the following code. Describe the output in the space below. 
**Answer**
We see our map of Maryland by zip code that is shaded based on our foreclosure data. We can generally see which zip codes had the most foreclosures in March 2023.
...
**Answer**
I've loaded the correct libraries and called in my census api key.
```{r}
library(tidyverse)
library(sf)
library(janitor)
...
**Task** Run the following code. Describe the output in the space below. 
**Answer**
We've created two new dataframes. The notices dataframe provides the number of foreclosures by county for March 2023. The population dataframe gives us population estimates by county and geoid.
...
**Task** Run the following code. Describe the output in the space below. 
**Answer**
We have our geoid, county name, estimated population and geometry data in this dataframe.
...
**Task** Run the following code. Describe the output in the space below. 
**Answer**
I see a dataframe with dates (March 2023), forclosures, counties and foreclosures by county. There's also a dataframe with out geometric information.
...
**Task** Run the following code. Describe the output in the space below. 
**Answer**
We've changed NAME to county and we see the population estimate, as well as the geometry column.
...
**Task** Run the following code. Describe the output in the space below. 
**Answer**
I see that we've joined the dataframes by county. We now have our foreclosure and county data merged with the geometry information.
...
**Task** Run the following code. Describe the output in the space below. 
**Answer**
I see basically the same dataframe as the previous codeblock, but this time we've added a column with the rate of foreclosure notices per 1000 population. This info is arranged from highest to lowest rates, with Charles County standing at the top.
...
**Task** Run the following code. Describe the output in the space below. 
**Answer**
I see a map of Maryland by county and shaded by rate from dark blue to light blue with the lighter blue counties signifying higher foreclosure rates per 1000 population. It's a bit hard to read the names of some counties.
...
**Task** Run the following code. Describe the output in the space below. 
**Answer**
This is the same map as the previous codeblock except we've changed the colors to a colorscheme that makes it much easier to discern the rate of foreclosures across Maryland. We've also removed the county names. 
...
**Answer**
We've removed a color from the scale and added more digits after the decimal point for the rates. I see that generally, the southern parts of Maryland have higher foreclosure rates compared to the northern regions. Charles County is clearly the county with the highest rate based on our legend. I also see that many counties bordering the Chesapeake Bay seem to be higher on the scale of foreclosure rates.
