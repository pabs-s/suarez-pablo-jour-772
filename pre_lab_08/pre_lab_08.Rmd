...
**Answer**
the baltco_911_calls dataframe has 3822 rows and four columns.
...
**Answer**
The first line is creating a new dataframe from the original baltco dataframe. Then it's mutating to create a new column called 'month' based on the original 'date' column. The "label=TRUE" part of that line is matching the month number to the corresponding month name, i.e. 1 = January. Then the code is grouping the data by our new month column, summarizing the total number of calls in those months and then arranging the total calls in descending order.
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer**
I see a bar chart with columns by month (x-axis) and a y-axis of the count that we found in task 3. 
...
**Answer**
The bar chart has been rearranged to show total calls in ascending order from left to right. I'm also noticing, starting from the last codeblock, that we're using a plus sign instead of a pipe.
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? 
**Answer**
The bar chart has been flipped from vertical to horizontal with the highest values starting at the top. This codeblock used the coord_flip function to do this.
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? 
**Answer**
I see a similar bar chart from the previous task. However, this one has a more transparent background grid. We've incorporated the theme_minimal function to make that change.
...
**Answer**
I see a new version of the same horizontal bar chart, and the axis labels now seem a little more crowded. This one has a different background. It has a grayish-blue hue and white horizontal lines underneath each bar, as opposed to a background grid. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? 
**Answer**
We've now added a title to our bar chart, cleaned up the axis labels by labeling them in the code and provided a source for our data through the caption portion of the labs function.
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer**
It's a new dataframe counting the Baltimore County calls by date (year, month, day)
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer**
I see a line chart using the previous task's data. The x-axis is the date, while the y-axis is the total number of calls. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer**
We've added more individual points on the x-axis with the scaling function. The result of this action has overcrowded the names along the x-axis.
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer**
The x-axis text has now been tilted at a 45 degree angle. It's significantly easier to read it.
