...
My URL: https://datawrapper.dwcdn.net/3K4eg/1/
...
md_gov_county <- 
  md_gov_county |>
  group_by(County) |>
    mutate(county_total = sum(Cox, Moore, Lashar, Wallace, Harding)) |>
    mutate(moore_percent = sum(Moore)/county_total*100) |>
  clean_names()
...
**Answer** 
I now just see a map of Maryland with various levels of shading based on the percentage column.
...
**Answer** 
Now when I hover over a county, it gives me the county's actual name instead of the zip code and gives me the percentage in the format we just updated.
...
https://datawrapper.dwcdn.net/9Lcys/1/
